...
export const CaseMemoArea = (props: FormatAreaProps) => {
  const { isOpen, setIsOpen } = useDisclosure(false);
  const { title, userCaseMemo, sampleCaseMemo, onUserTextChange } = props;
  ...
  return (
    <div className="flex flex-col bg-white">
      <div className="flex items-center justify-between bg-[#F9F7F4] pl-2">
      ...
      <div className={`min-h-output-textarea flex`}>
        <textarea
          ...
          defaultValue={
            userCaseMemo
              ? formatTextWithNewLine(userCaseMemo)
              : formatTextWithNewLine(sampleCaseMemo)
          }
        />
        <p
        ...
        >
          {formatTextWithBreaks(sampleCaseMemo)}
        </p>
      </div>
    </div>
  );
