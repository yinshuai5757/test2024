...
export const GeneratedArea = (props: FormatAreaProps) => {
  const { isOpen, setIsOpen } = useDisclosure(false);
  const { title, userGeneratedText, sampleGeneratedText, onUserTextChange } =
    props;
    ...
    <div className="flex flex-col bg-white">
      ...
      <div className={`min-h-output-textarea flex`}>
        <textarea
          ...
          defaultValue={
            userGeneratedText
              ? formatTextWithNewLine(userGeneratedText)
              : formatTextWithNewLine(sampleGeneratedText)
          }
        />
        <p
        ...
        >
          {formatTextWithBreaks(sampleGeneratedText)}
        </p>
      </div>
    </div>
  );
